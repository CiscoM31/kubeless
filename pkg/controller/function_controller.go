...
type FunctionController struct {
	...
	smclient         *monitoringv1.MonitoringV1Client
...
func NewFunctionController(cfg Config, smclient *monitoringv1.MonitoringV1Client) *FunctionController {
...
func (c *FunctionController) startImageBuildJob(funcObj *kubelessApi.Function, or []metav1.OwnerReference) (string, bool, error) {
	...
	imagePullSecret, err := c.clientset.CoreV1().Secrets(funcObj.ObjectMeta.Namespace).Get(ctx, "kubeless-registry-credentials", metav1.GetOptions{})
	if err != nil {
...
func (c *FunctionController) deleteK8sResources(ns, name string) error {
	...
	err := c.clientset.ExtensionsV1beta1().Deployments(ns).Delete(ctx, name, metav1.DeleteOptions{PropagationPolicy: &deletePolicy})
	if err != nil && !k8sErrors.IsNotFound(err) {
		return err
	}
	// delete svc
	...
	// delete build job
	err = c.clientset.BatchV1().Jobs(ns).DeleteCollection(ctx, metav1.DeleteOptions{}, metav1.ListOptions{
